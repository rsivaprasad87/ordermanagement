...
This project demonstrates how to build and deploy a **Spring Boot Application** integrated with multiple **AWS services**. It is designed as a step-by-step beginner-friendly learning project.
...
5. The document url is updated for the customer in RDS   
...
- After terraform apply is complete, all the required environment variables can be found using command "terraform output"
...
| `Profile file contained no credentials for profile 'default'` | SDK trying to load local AWS profile on EB                                 | Use `InstanceProfileCredentialsProvider` to use IAM role credentials  //.credentialsProvider(ProfileCredentialsProvider.create()) - Used  only for local testing and .credentialsProvider(InstanceProfileCredentialsProvider.create()) to be used for application running in Elastic Beanstalk |
| `AccessDenied: elastic load balancing`                       | EB service role lacks permission                                           | Attach `AWSElasticBeanstalkManagedUpdatesCustomerRolePolicy` to EB role. Also application runs on port 8080. Default port of Load balcner is 80 .So either set the server.port in application.yml as 80 or make Load Balancer forward the request to 8080                                      |
...
## 8. Useful Tips
- Refer https://docs.aws.amazon.com/sdkref/latest/guide/file-location.html --> AWS credentials 
...
- Once the connection is established use the below commands with RDS 
  ...
  show tables;
  show columns from customer; <your entity name >
